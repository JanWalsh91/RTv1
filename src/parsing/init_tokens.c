...
{
	if (!(t->tokens = (char **)ft_memalloc(T_COUNT)))
		ft_errno_exit();
	t->tokens[T_OPEN_BRACKET] = ft_strdup("{");
	...
	t->tokens[T_AMBIENT_LIGHT_COLOR] = ft_strdup("ambient light color");
...
static void	init_tokens2(t_parse_tools *t)
{
	...
	t->tokens[T_READ_MATERIAL_FILE] = ft_strdup("read material file");
